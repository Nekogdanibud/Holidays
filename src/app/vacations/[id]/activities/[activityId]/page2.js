...
export default function ActivityDetailPage() {
  ...
  const fetchActivity = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Находим участие текущего пользователя
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      // Добавляем баннер если выбран новый
      ...
      if (response.ok) {
        ...
        // Переключаемся на вкладку "info" после успешного сохранения
        setActiveTab('info');
        setIsEditing(false);
        setBannerPreview(null);
        setFormErrors({});
        
        // Сохраняем состояние участия пользователя
  ...
  const handleEditChange = (e) => {
    ...
    // Очищаем связанные ошибки местоположения
  ...
  // Получаем только участников, которые идут
  ...
  // Проверяем, является ли пользователь владельцем или организатором
  ...
  // Функция для определения статуса участия пользователя
  ...
  // Используем эту функцию для определения состояния
          ...
          {activeTab === 'participants' && (
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                {/* Кнопки участия в разделе участников */}
                <div className="flex items-center space-x-3">
                  {currentUserParticipation?.status === 'GOING' ? (
                    <button
                      onClick={() => handleParticipation(false)}
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  onClick={() => {
                    if (isEditing) {
                      // Если отменяем редактирование - переходим на вкладку info
                      setActiveTab('info');
                    }
                  ...
                  <div className="flex space-x-3 pt-4">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setBannerPreview(null);
                        setFormErrors({});
                        setActiveTab('info'); // Переходим на вкладку info при отмене
                      }}
                      disabled={isSaving}
